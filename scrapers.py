...
def deviant_art(art_url):
    ...
    driver.get(
        "https://tvchecklist.com/404"
    )  # need to navigate to a page before setting a cookie :/
    try:
        with open(deviantart_cookies, "rb") as file_to_be_read:
            cookies = msgpack.unpack(
                file_to_be_read, raw=False
            )  # without raw=False everything is prefixed with a "b"
    ...
    artist = driver.find_element(
        By.XPATH,
        "//*[@id='output']/div/div[3]/div[1]/div[2]/div/div[1]/h1/small/span[2]/a",
    )
    ...
    try:
        image_download_link = driver.find_element(
            By.XPATH, '//a[contains(@class, "dev-page-download")]'
        ).get_attribute("href")
        driver.get(
            image_download_link
        )  # since image_download_link is a redirect, we need to resolve it to get the direct link
        art["image_url"] = driver.current_url
    except:
        try:
            art["image_url"] = driver.find_element(
                By.CLASS_NAME, "dev-content-full"
            ).get_attribute("src")
            ...
            try:  # part from https://github.com/Romitas/DeviantArtScraper/blob/master/deviant/spiders/deviant_spider.py
                ...
                submit = driver.find_element(
                    By.XPATH, '//input[contains(@class, "submitbutton")]'
                )
                ...
                try:
                    ...
                    image_download_link = WebDriverWait(driver, 10).until(
...
def art_station(art_url, multiple=False):
    ...
    if not multiple:
        ...
        for art_data_asset in art_data["assets"]:
            if (
                art_data_asset["has_image"]
                and not art_data_asset["has_embedded_player"]
            ):
...
def pixiv(art_url, username, password):
    ...
    try:  # check if the user not logged in - if not, then log in
        driver.find_element(
            By.XPATH, '//a[contains(@class, "ui-button _login")]'
        ).click()
        driver.find_element(
            By.XPATH, '//*[@id="LoginComponent"]/form/div[1]/div[1]/input'
        ).send_keys(username)
        driver.find_element(
            By.XPATH, '//*[@id="LoginComponent"]/form/div[1]/div[2]/input'
        ).send_keys(password)
    ...
    try:
        art["title"] = driver.find_element(
            By.XPATH,
            '//*[@id="root"]/div[1]/div/div/article/div[1]/figure/figcaption/div/div/h1',
        ).text
        artist = driver.find_element(
            By.XPATH, '//*[@id="root"]/div[1]/div/div/aside/section/div[1]/div/a'
        )
        art["artist_name"] = artist.text
        art["artist_website"] = artist.get_attribute("href")
        art["source"] = art_url
        art["image_url"] = driver.find_element(
            By.XPATH,
            '//*[@id="root"]/div[1]/div/div/article/div[1]/figure/div[1]/div/a',
        ).get_attribute("href")
    except Exception as e:
        print(e)
...
def twitter(art_url):
    ...
    art["artist_website"] = (
        "https://twitter.com" + soup.find("a", ["js-action-profile"])["href"]
    )
    art["source"] = art_url
    art["image_url"] = soup.find("meta", property="og:image")["content"]
