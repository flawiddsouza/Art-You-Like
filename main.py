...
def get_all_art_data(db, sort_by=None, count=51, offset=0):
    order = (
        "ORDER BY a.title DESC" if sort_by == "title" else "ORDER BY a.updated_at DESC"
    )
    rows = db.execute(
        f"{_BASE_ART_SELECT} {order} LIMIT ? OFFSET ?", [count, offset]
    ).fetchall()
...
def get_all_art():
    ...
    try:
        count = int(request.args.get("count", 51))
...
def add_art():
    ...
    for local_image in request.form.getlist("local-image"):
        if local_image == "true":
            ...
            if file and helpers.allowed_file(file.filename, ALLOWED_EXTENSIONS):
                filename = helpers.prepend_date_time_to_string(
    ...
    if request.form.get("existing-artist") == "true":
        ...
        cursor = g.db.execute(
            "INSERT INTO artist(name,website) VALUES(?,?)",
            (request.form.get("artist-name"), request.form.get("artist-website")),
        )
        artist_id = cursor.lastrowid

    cursor = g.db.execute(
        "INSERT INTO art(title,artist_id,source) VALUES(?,?,?)",
...
def edit_art():
    ...
    old_urls = {
        ...
        for r in g.db.execute(
            "SELECT url FROM art_image WHERE art_id=?", [id]
        ).fetchall()
    }
    ...
    g.db.execute(
        "UPDATE art SET title=?,artist_id=?,source=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",
...
def delete_art():
    ...
    for r in g.db.execute("SELECT url FROM art_image WHERE art_id=?", [id]).fetchall():
        try:
            os.remove(os.path.join(UPLOAD_FOLDER, r["url"]))
        except Exception:
            pass
...
def delete_art_image():
    ...
    g.db = connect_db()
    try:
        os.remove(os.path.join(UPLOAD_FOLDER, image_url))
    except Exception:
        pass
    try:
...
def get_all_artists():
    ...
    artists = [
        dict(id=row[0], name=row[1], website=row[2], added_at=row[3], updated_at=row[4])
...
def get_all_tags():
    ...
    artists = [
        dict(id=row[0], name=row[1], added_at=row[2], updated_at=row[3])
        for row in cursor.fetchall()
    ]
    g.db.close()
    return jsonify(artists)
...
def add_from(request, prefix_lower, prefix_normal, multi=False):
    ...
    if not url:
        if helpers.request_wants_json():
            return jsonify(
                status="error", message=f"{prefix_normal} Image url was empty"
            )
    ...
    scraper_map = {
        ...
        "pixiv": lambda: scrapers.pixiv(
            url,
            load_settings().get("pixiv_username"),
    ...
    if multi:
        images = [
            helpers.download(img_url, UPLOAD_FOLDER) for img_url in art["image_url"]
        ]
    ...
    if request.form.get("existing-artist"):
        ...
        row = g.db.execute(
            "SELECT id FROM artist WHERE website=?", [art["artist_website"]]
        ).fetchone()
        if row:
            artist_id = row["id"]
        else:
...
def edit_tag():
    ...
    g.db = connect_db()
    g.db.execute(
        "UPDATE tag SET name=?, updated_at=CURRENT_TIMESTAMP WHERE id=?", [name, id]
    )
    g.db.commit()
    g.db.close()
...
def artist_manager():
    ...
    artists = [
        ...
        for r in g.db.execute(
            "SELECT id, name, website, created_at, updated_at FROM artist"
        ).fetchall()
    ]
    counts = {
        r["artist_id"]: r["cnt"]
...
def edit_artist():
    ...
    g.db = connect_db()
    g.db.execute(
        "UPDATE artist SET name=?, website=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
        [name, website, id],
    )
    g.db.commit()
    g.db.close()
...
def delete_artist():
    ...
    g.db = connect_db()
    for r in g.db.execute(
        "SELECT ai.url FROM art_image ai JOIN art a ON ai.art_id=a.id WHERE a.artist_id=?",
        [id],
    ).fetchall():
        try:
            os.remove(os.path.join(UPLOAD_FOLDER, r["url"]))
...
def search_json():
    ...
    try:
        ...
        offset = 0
    return jsonify(
        get_search_results(query, count=count, offset=offset) if query else []
    )
...
def search():
    ...
    return render_template(
        "search.html", art=art, search_query=query, layout=load_settings().get("layout")
    )
