...
def seed_test_db(db_path):
    with sqlite3.connect(db_path) as conn:
        create_schema(conn)
        conn.execute(
            "INSERT INTO artist(id,name,website) VALUES(1,'Test Artist','http://example.com')"
        )
        conn.execute(
            "INSERT INTO art(id,title,artist_id,source) VALUES(1,'Test Art',1,'http://source.com')"
        )
        conn.execute(
            "INSERT INTO art_image(art_id,url,position,width,height) VALUES(1,'img1.jpg',0,1920,1080)"
        )
        conn.execute(
            "INSERT INTO art_image(art_id,url,position,width,height) VALUES(1,'img2.jpg',1,800,600)"
        )
