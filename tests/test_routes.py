...
def test_artist_manager_art_count_correct(client, test_db_path):
    ...
    with sqlite3.connect(test_db_path) as conn:
        conn.execute(
            "INSERT INTO artist(id,name,website) VALUES(2,'Artist B','http://b.com')"
        )
        conn.execute(
            "INSERT INTO art(id,title,artist_id,source) VALUES(2,'Art 2',1,'http://s.com')"
        )
        conn.commit()
...
def test_add_art_stores_image_dims(client, test_db_path):
    with (
        patch("helpers.download", return_value="new_img.jpg"),
